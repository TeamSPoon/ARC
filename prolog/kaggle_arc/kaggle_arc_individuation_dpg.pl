...
:- abolish(individuation_macros/2).
...
:- discontiguous(fti/2).
:- abolish(run_fti/2).
:- discontiguous(run_fti/2).

%amass(I,M):-mass(I,M).
colors(I,CC):-colors_cc(I,CC).
...
ig(ROptions,Grid):-
  ...
  locally(nb_setval(debug_indiv,nil),show_individuated_nonpair(ig,ROptions,Grid,GridIn,IndvS)).
...
ig_test_id_num_io(ROptions,GridIn,_ID,TestID,_Example,_Num,_IO,IndvS):- 
  ...
  my_time(individuate_3(ROptions,GridIn,IndvS)),!.
...
show_individuated_nonpair(PairName,ROptions,GridIn,Grid,InC):-
 must_det_ll((
  print_list_of(debug_indiv,PairName,InC),!,
  length(InC,Len),
...
really_show_touches(Title,InShown,Obj):- 
  debug_indiv(really_show_touches(Title),Obj),
...
% if there are 10 or less of a color dont group the whole color (It/they must be special lo_points)
...
individuation_macros(altro, [
    reduce_population,
    remove_used_points,
    when((len(lo_points)=<ThreeO),alone_dots(lte(5))),    
    when((len(lo_points)>ThreeO),by_color)]):- the_big_three_oh(ThreeO).
...
individuation_macros(do_ending, [
  %find_edges,
  % find_contained_points, % mark any "completely contained lo_points"
...
individuation_macros(completely_do(This),[This,maybe_lo_dots,do_ending]).
...
% % %%    individuator(i_common,[common_shape_lib,do_ending]).
  ...
  %TODO when(len(objs)<70,when(len(lo_points)<50,glyphic)),
  %do_ending,
...
individuation_macros(complete2, [
    ...
    %when((colors(i.lo_points,Cs),len(Cs)<2),alone_dots(lte(5))), % any after this wont find individuals unless this is commented out
    colormass_merger(2),
    when((len(lo_points)=<ThreeO),alone_dots(lte(5))),
    alone_dots(lte(5)),
...
% =====================================================================
remove_if_prop(Prop,VM):- my_partition(has_prop(Prop),VM.objs,_With,Rest),gset(VM.objs)=Rest.
...
:- ensure_loaded(kaggle_arc_individuation_cbox_dpg).
% =====================================================================
...
hybrid_shape_from(Set,VM):-
 ...
 %wdmsg(maybe_ogs_color(R,OH,OV,In,Grid)),
  ...
  intersection(VM.lo_points,GOPoints,Intersection,LeftOver,Missing),
  Missing\==[],
  Intersection\==[],
...
%detect_indvs(In,Out,Grid):- individuate_3(InHow,Grid,Out).
...
get_individuated_cache(_TID,ROptions,GOID,IndvS):- nonvar(ROptions),
  ground(GOID), \+ luser_getval(individuated_cache,false), individuated_cache(_,GOID,ROptions,IndvS),!.

get_individuated_cache(_TID,ROptions,GOID,IndvS):- nonvar(ROptions),
  ground(GOID), \+ luser_getval(individuated_cache,false), saved_group(individuate_3(ROptions,GOID),IndvS),!.
...
individuate_two_grids_once(OID_In_Out,ROptions,GridIn,GridOut,IndvSI,IndvSO):- 
  %wdmsg(individuate_3(ROptions,OID_In_Out,IndvSI,IndvSO)),
  (saved_group(individuate_3(ROptions,OID_In_Out),IndvS)
...
individuate_two_grids_now(OID_In_Out,ROptions,Grid_In,Grid_Out,ID_In,ID_Out,P_In,P_Out,IndvSI,IndvSO):-
  ...
  if_t((H_In==H_Out,V_In==V_Out),((
      ...
      if_t((U_In\==[],U_Out==[]),(set(VM_In.lo_points)=U_In,set(VM_In.start_points)=U_In,
                            points_to_grid(H_In,V_In,U_In,Grid_InX),set(VM_In.grid)=Grid_InX,set(VM_In.start_grid)=Grid_InX)),
...
individuate2(VM,ROptions,GOID,GridIn,IndvS):-
  ...
  =(LF,IndvS),
  if_t(nonvar(GOID),
   (retractall(individuated_cache(_,GOID,ROptions,_)), 
    length(IndvS,Len),ignore((Len>0,atom(ROptions),progress(yellow,oid_created(ROptions,GOID,len(Len),'$VAR'(Len))))),
    my_asserta_if_new(individuated_cache(TID,GOID,ROptions,IndvS)))),!.
...
% tiny grid becomes a series of lo_points
...
into_fti_hide(ID,ROptions,GridIn0,VM):-
...
run_fti(_,[Done|TODO]):- my_assertion(nonvar(Done)), ( \+ done \= Done ), !, wdmsg(done_run_fti([Done|TODO])),set_vm(lo_program,[done]),!.
run_fti(VM,[if_done|TODO]):- !, (VM.lo_points==[] -> (wdmsg(if_done),set_vm(lo_program,[if_done])) ; run_fti(VM,TODO)).
...
print_vm_debug_objs(VM):- 
  Objs = VM.objs,  
  length(Objs,Count),
  as_debug(8,(amass(Objs,Mass), length(VM.lo_points,PC),
      maybe_four_terse(VM.lo_program,Four),
      progress(t([obj/obj_mass=(Count/Mass),unprocessed_points=PC,roptions=VM.start_options,fsi=Four])))).
...
fti(VM,[call(G)|TODO]):-   set(VM.lo_program) = TODO, !, my_submenu_call(call_expanded(VM,G)).
...
%fti(VM,[Step|Program]):- \+ missing_arity(Step,0), !, set(VM.lo_program) = Program, my_submenu_call(call(Step)).
...
bg_shapes(Shape,VM):-
 must_det_ll((
  ...
  maplist(recolor_object(VM,Orig),FoundObjs,ReColored),
...
fg_shapes(Shape,VM):-
 ...
 must_det_ll((
  ...
  set_vm(VMS),
  globalpoints_include_bg(VM.start_grid,Recolors),
  maplist(recolor_object(VM,Recolors),FoundObjs,ReColored),
  learn_hybrid_shape(ReColored),
  remCPoints(VM,ReColored),
  remGPoints(VM,ReColored),
  addInvObjects(VM,ReColored))))),!.
...
fg_subtractiond(This,Target,Cell):- \+ This \= Target,!,get_black(Cell).
fg_subtractiond(This,_,Cell):-  get_black(Cell), \+ This \= Cell,!.
%fg_subtractiond(This,Target,This):- get_black(Cell), \+ Target \= Cell,!.
...
fg_subtractions(Subtraction,VM):-
 ...
 must_det_ll((
  must_be_free(GOID),atomic_list_concat([VMGID,'_fg_subtractiond'],GOID), assert_grid_gid(NewGrid,GOID),
  get_vm(VMS), 
  %individuate2(_,Subtraction,GOID,NewGrid,FoundObjs),
  with_other_grid(Other,individuate_3(Subtraction,NewGrid,FoundObjs)),
  set_vm(VMS),
  ReColored = FoundObjs,
...
non_intersectiond(Cell,Target,Target):- get_black(Cell),!.
non_intersectiond(Target,Cell,Target):- get_black(Cell),!.
non_intersectiond(This,Target,Cell):- This =@= Target, get_black(Cell), !.
...
fg_intersections(Intersection,VM):-
 ...
 must_det_ll((
  must_be_free(GOID),atomic_list_concat([VMGID,'_fg_intersectiond'],GOID), assert_grid_gid(NewGrid,GOID),
  get_vm(VMS), 
  %individuate2(_,Intersection,GOID,NewGrid,FoundObjs),
  with_other_grid(Other,individuate_3(Intersection,NewGrid,FoundObjs)),
  set_vm(VMS),
  ReColored = FoundObjs,
  %globalpoints_include_bg(VM.start_grid,Recolors), maplist(recolor_object(Recolors),FoundObjs,ReColored),
...
fg_abtractions(Subtraction,VM):-
 ...
 must_det_ll((
  must_be_free(GOID),atomic_list_concat([VMGID,'_fg_abtractiond'],GOID), assert_grid_gid(NewGrid,GOID),
  get_vm(VMS), 
  %individuate2(_,Subtraction,GOID,NewGrid,FoundObjs),
  individuate_3(Subtraction,NewGrid,FoundObjs),
  set_vm(VMS),
  ReColored = FoundObjs,
  %globalpoints_include_bg(VM.start_grid,Recolors), maplist(recolor_object(Recolors),FoundObjs,ReColored),
...
drops_as_objects(Name,VM):-
 ...
 must_det_ll((
  Grid = VM.grid,
  %notrace(catch(call_with_depth_limit(individuate1(_,Name,VM.start_grid,IndvS0),20_000,R),E,(wdmsg(E)))),
...
group_prior_objs(Why,ObjsIn,WithPriors):- 
 ...
 %print_list_of(debug_indiv,group_prior_objs,Objs),!,
 group_priors(Objs,Lbls),
 keysort(Lbls,N),
 length(N,Len),
 Title = Why+Len,
 print_premuted_objects(Title,Objs), 
   ...
   (length(IndvS0,Len),
    if_t(Len>0,
     ...
     /*
      ...
      nop(( DebugThis\==false -> Feedback = debug_indiv(Title) ; Feedback = print_info)),
...
% alone_dots(lte(5)) that have no adjacent lo_points of the equal color (could be gathered first)
...
using_alone_dots(VM, _):- \+ contains_alone_dots(VM.start_grid), \+ contains_alone_dots(VM.target_grid),!.
...
is_fti_step(lo_dots).
% =====================================================================
% lo_dots may have adjacent lo_points of the equal color (because we are in 'lo_dots' mode)
...
one_fti(VM,glyphic):-
 must_det_ll((
  ...
  using_alone_dots(VM,(maplist(make_point_object(VM,[birth(glyphic),iz(shaped)]),UPoints,IndvList), assumeAdded(VM,IndvList),
...
whole_into_obj(VM,Grid,Whole):- 
  ...
  if_t(Len>0,
    (make_indiv_object(VM,[amass(Len),vis2D(H,V),birth(whole),iz(always_keep),loc2D(1,1),iz(always_keep),iz(image)|Props],Points,Whole),assumeAdded(VM,Whole),
  ...
  length(LPoints,CLen),if_t((CLen=<144,CLen>0),    
    (make_indiv_object(VM,[birth(whole),iz(shaped),loc2D(1,1)],LPoints,Whole2),assumeAdded(VM,Whole2))).
...
release_points(VM):- 
    ...
    re_addLeftOverPoints(VM,NextScanPoints1),
    globalpoints(VM.objs,NextScanPoints2),
    re_addLeftOverPoints(VM,NextScanPoints2).
...
assumeAdded(VM,Obj):- nop(assertion(member(Obj,VM.objs))).
...
re_addLeftOverPoints(_VM,Obj):- Obj==[],!.
re_addLeftOverPoints(VM, Obj):- assume_vm(VM),!,globalpoints(Obj,CPoints), 
  intersection(VM.lo_points,CPoints,PretendToAdd,Prev,ReallyAdd),
...
one_fti(VM,Option):- 
  (one_fti_1(VM,Option)->true;(u_dmsg(missed(one_fti(Option))))).
