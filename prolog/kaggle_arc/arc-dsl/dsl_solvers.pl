...
read_sols:- setup_call_cleanup(open('arc-dsl/solvers800.py',read,In2,[encoding(octet)]),read_python(In2),close(In2)),!.
read_dsl_test:- uast,!,
     setup_call_cleanup(open('arc-dsl/constants.py',read,In1,[encoding(utf8)]),read_python(In1),close(In1)), !.
