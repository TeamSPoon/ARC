...
pass2_rule2(TestID,Ctx,edit(Type,different,[P],P),[iz(info(propcan(true,Ctx)))|PSame]):- fail,
...
% mutiple postconds
trans_rule(Info,In,[Out,Out2|OutL],TransRule):- trace, is_object(Out),is_object(Out2),
...
trans_rule(Info,[In],[Out],Rules):- 
  ...
  Rules = [ copy_if_match(Info,rhs(copy_step1(Step,TypeO)),lhs(Same)) ],!.

% just copy an object
trans_rule(Info,In,Out,Rules):- 
  sub_compound(step(Step),Info), sub_compound(why(TypeO),Info),
  noteable_propdiffs(In,Out,Same,L,R),L==[],R==[],
  Rules = [ copy_if_match(Info,rhs(copy_step2(Step,TypeO)),lhs(Same)) ],!.


% copy/transform 
trans_rule(Info,E1,E2,Rules):- 
  Rule = edit_copy(Info,rhs(edit(Type,Change,[step=step(Step),typeO=why(TypeO),known=S,unk=L,nr=NR,rhs=RHS,set=Set],P)),lhs(LHS)),
 must_det_ll((
  noteable_propdiffs2(E1,E2,S,L,RR),
  sub_compound(step(Step),Info),
  sub_compound(why(TypeO),Info))),
  into_lhs(E1,LHS),  
  my_partition(good_for_rhs,RR,RHS,NR),
  if_t(how_are_differnt(E1,E2,Set),pp_ilp(how_are_differnt=Set)),
  pp_ilp(Rule),
 must_det_ll(( findall(Rule,
    (member(P,RHS),prop_pairs(E1,E2,Type,Change,P),
      Change\==same,
...
trans_rule(Info,E1,E2,Rules):-
  ...
  pp_ilp(info=Info), pp_ilp(nremoved=NL), pp_ilp(nsames=NSame), pp_ilp(nadded=NR),
  sub_compound(step(Step),Info), sub_compound(why(TypeO),Info),
  dash_chars,
  Rules = [ 
