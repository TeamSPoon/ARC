...
maybe_cbox_vm(VM):- var(VM.option_cboxes), !, 
 (need_cboxes(VM.start_grid) -> (set(VM.option_cboxes)=true) ; (set(VM.option_cboxes)=false)),
  maybe_cbox_vm(VM).
...
cbox_vm(VM):- !,
  ...
  if_t(Points\==Points2,set(VM.lo_points)=Points2),
  ...
  if_t(Points\==Points9,set(VM.lo_points)=Points9),
  !.
...
cbox_pair(TestID,ExampleNum,I,O):-
   u_dmsg(?- test_p2(cbox_pair(TestID,ExampleNum))),
...
cbox_io(TestID,ExampleNum,IO,G0):-
  ...
  u_dmsg(?- cbox_io(TestID,ExampleNum,IO)),
...
i_cbox_l(_Grid,_NSEW,_XSG,Points,Points,_VM,L_S,_):- Points==[], !, u_dmsg(pointless(L_S)).
i_cbox_l(_Grid,_NSEW,_XSG,Points,Points,_VM,L_S,[]):- !, u_dmsg(complete(L_S)).
