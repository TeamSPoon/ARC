...
not_assumed(P):- is_unbound_prop(P),!,fail.
...
solve_obj_group(VM,TestID,_ExampleNum,Ctx,ObjsIn,ObjsO):-
  ...
  rules_no_assumed(Many),
  findall_vset_R(Rule,member(ac_unit(TestID,Ctx,P,PCond),Many), Rules),
...
into_object_dependancy_r_l(TestID,ExampleNum,Ctx,RHSObjs,LHSObjs,Groups):-
  normalize_objects_for_dependancy(RelaxedLvl,TestID,ExampleNum,RHSObjs,LHSObjs,RHSO,LHSO),!,
...
normalize_objects_for_dependancy(RelaxedLvl,TestID,ExampleNum,RHSObjs,LHSObjs,RHSObjsO,LHSObjsO):- 
  ...
  normalize_objects_for_dependancy(RelaxedLvl,TestID,ExampleNum,RHSObjsM,LHSObjsM,RHSObjsO,LHSObjsO).

normalize_objects_for_dependancy(RelaxedLvl,_TestID,_ExampleNum,RHSObjs,LHSObjs,RHSO,LHSO):- fail,
  ...
  !.
normalize_objects_for_dependancy(RelaxedLvl,_,_,L,R,L,R):-!.
...
into_rhs(P,E):- sub_cmpd(rhs(E),P),!. 
into_rhs(rhs(R),P):- !, into_rhs(R,P).
into_rhs([R],P):- !, into_rhs(R,P).
into_rhs(l2r(_Tst,_IO,P),Out):- into_rhs(P,Out),!.
into_rhs(P,P).
...
is_unbound_prop(S):- make_unifiable(S,DS), S=@=DS,!, \+ ground(S).
...
verify_object(Info,Obj):-
 % my_assertion(localpoints(Obj,_LP)),
 % my_assertion(globalpoints(Obj,_GP)),
 nop((ignore(show_bad_objs(Info,Obj)),
  nop(assertion((iz(Obj,symmetry_type(What)), nonvar(What)))))).
...
center2D(I,X,Y):- I=size2D(H,V),  X is floor(H/2),Y is floor(V/2).
center2D(I,X,Y):- arg(1,I,X),arg(2,I,Y),number(X),number(Y),!.
...
center2D(Points, CenterX, CenterY) :- maplist(center2D,Points,X,Y),
  length(Points,Count), sumlist(X,SumX),sumlist(Y,SumY),
   ...
   CenterX is floor(SumX / Count)+1, CenterY is flor(SumY / Count)+1.
