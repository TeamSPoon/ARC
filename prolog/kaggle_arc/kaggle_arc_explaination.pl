...
show_indiv_textinfo2(Why,AS0,ExceptFor):-
 must_det_ll((
  ...
  % pp(vAS0=AS0),
  extend_l2r_proplist([obj(Props11)],[obj(Props)]),
...
print_body(D,Head, (:- Conj)):- !, conjuncts_to_list(Conj,List),  
  prefix_spaces(D, (writeln(':- '),
    if_t(menu_or_upper('E'), if_t(\+ any_body_oids(Conj), (which_body_oids(Conj,OIDs),print_ss(OIDs)))),
...
print_global_ngrid(OID):- into_global_ngrid(OID,Grid),print_grid(Grid).

into_global_ngrid(OID,print_grid(OID,FGGrid)):- 
   oid_to_obj(OID,Obj),global_grid(Obj,Grid),into_solid_grid(Grid,SGrid),
