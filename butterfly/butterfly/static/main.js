      ...
      encode = (function (_this) {
        return function (data, ch) {
          if (!_this.utfMouse) {
            ...
            if (ch > 160) {
              //console.log("PUSH = &#"+ch+";");
              //return data.push("<span class='mc-10'>&#" + ch + ";</span>");
            }
            if (ch < 127) {
              return data.push(ch);
            } else {
    ...
    Terminal.prototype.screenToDom = function (force) {
      ...
      for (y = k = 0, len = ref.length; k < len; y = ++k) {
        ...
        if (line.dirty || force) {
          ...
          var leftOver = jarj.split("&nbsp;").join(' ').split(" ").join('');
          if (leftOver == '') {
            blankLines++;
            if (blankLines == 1) {
              continue;
          ...
          const tenSp = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
          const fourtySp = tenSp + tenSp + tenSp + tenSp + tenSp + tenSp+ '&nbsp;&nbsp;'
          jarj = jarj.split(fourtySp).join(tenSp);
          jarj = jarj.split(fourtySp).join(tenSp);
          ...
          */
          if (!active) {
            if (jarj.indexOf("<pre") == -1) {
              if (jarj.indexOf('pl-fun') > -1 || jarj.split("</span>").join("").split("<span ").join("").indexOf('<') > -1) {
